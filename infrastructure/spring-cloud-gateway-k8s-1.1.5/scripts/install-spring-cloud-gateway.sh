...
readonly DEFAULT_REPLICA_COUNT=1
...
helm_upgrade() {
  ...
  if [[ "$#" -ge 4 ]]; then
    ...
    helm upgrade --install --atomic --create-namespace \
      --namespace "$namespace" \
      --set scg-operator.image="$operator_image",gateway.image="$gateway_image",scg-operator.registryCredentialsSecret="$registry_credentials_secret",scg-operator.replicaCount="${REPLICA_COUNT}" \
      --timeout="${TIMEOUT}" \
    ...
    helm upgrade --install --atomic --create-namespace \
      ...
      --timeout="${TIMEOUT}" \
      --set="scg-operator.replicaCount=${REPLICA_COUNT}" \
      "$chart_name" "$chart_tarball_path" && check_status || error_diagnostic
  fi
}
...
main() {
  ...
  local REPLICA_COUNT="$DEFAULT_REPLICA_COUNT"
  while [[ $# -gt 0 ]]; do
    key="$1"
    case $key in
      --operator_image)
